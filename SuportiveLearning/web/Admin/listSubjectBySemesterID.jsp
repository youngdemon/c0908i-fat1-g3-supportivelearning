...
<jsp:useBean id="objAccount" class="DAL.Admin.ManagementAccount">
    <jsp:setProperty name="objAccount" property="studentId" value="${studentId}"/>
</jsp:useBean>
<div id="header2">List Subjects</div>
<ol>
    <ul>
        <c:forEach var="item" items="${objSubject.subjectBySemesterId}">
            <li><a href="StudentListAssignmentRedirect.do?batchId=${objAccount.batchIdByStudentId}&subjectId=${item.subjectId}">${item.subjectName}</a></li>
        </c:forEach>
    </ul>
</ol>