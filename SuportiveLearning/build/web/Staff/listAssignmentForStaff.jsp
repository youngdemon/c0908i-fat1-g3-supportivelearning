...
<table border="0" width="100%">
    ...
    <c:forEach var="item" items="${items}">
        ...
        <td><input type="checkbox" name="ckbDelete"/></td>
    </tr>
    </c:forEach>
    <tr id="rowTable">
        <td colspan="6">
<c:if test="${empty requestScope.batchId && empty requestScope.subjectId}">
    <a href="AddNewAssignmentRedirect.do?batchId=${param.batchId}&subjectId=${param.subjectId}">Add New Assigment</a>
</c:if>
<c:if test="${not empty requestScope.batchId && not empty requestScope.subjectId}">
    <a href="AddNewAssignmentRedirect.do?batchId=${requestScope.batchId}&subjectId=${requestScope.subjectId}">Add New Assigment</a>
</c:if>
        </td>
    </tr>
</table>
